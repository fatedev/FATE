...
;		
;------------------------------------------------------------------------------
@if exp="typeof global.scenesave_object == 'undefined'"
@iscript
Scripts.execStorage("subroutine.tjs");

class SceneSavePlugin extends KAGPlugin
{
	var window;
	...
	function SceneSavePlugin(window)
	{
		super.KAGPlugin();
		this.window	= window;
	}
...
kag.addPlugin(global.scenesave_object = new SceneSavePlugin(kag));
...
;マクロを登録する
@macro name="tvoff"
@eval exp="scenesave_object.init(mp)"
@endmacro

@macro name="wtvoff"
@if exp="scenesave_object.doing"
@waittrig * name="tvoff" onskip="scenesave_object.finish()" canskip=%canskip|true
@endif
@endmacro
