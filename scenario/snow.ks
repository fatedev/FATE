...
class SnowPlugin extends KAGPlugin
{
	...
	function onExchangeForeBack()
	{
		...
		var snowcount = snows.count;
		for(var i = 0; i < snowcount; i++)
			snows[i].exchangeForeBack(); // move メソッドを呼び出す
	}
}
...
@eval cond="mp.nocheck!=1" exp="f.snow=1"
;@trace exp="mp.nocheck"
;@trace exp="mp.nocheck!=1"
;@trace exp="f.snow"
@endmacro
@macro name="snowuninit"
@eval exp="snow_object.uninit()"
@eval cond="mp.nocheck!=1" exp="f.snow=0"
;@trace exp="mp.nocheck"
;@trace exp="mp.nocheck!=1"
;@trace exp="f.snow"
@endmacro
...
@endmacro
;降雪を一時停止する(降雪中であれば)
@macro name="snow_suspend"
;@trace exp="f.snow"
@snowuninit cond="f.snow==1" nocheck=1
@endmacro
;降雪を再開する(降雪を一時停止中であれば)
@macro name="snow_recover"
;@trace exp="f.snow"
@snowinit cond="f.snow==1" nocheck=1
@endmacro
@return
